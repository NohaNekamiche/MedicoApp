...
class TreatmentAdapter (val context: Context, var data:MutableList<TraitementResponse>): RecyclerView.Adapter<Holder>() {
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        holder.nom_doc.text="Dr "+ data[position].name+" "+data[position].username
        ...
        idCardView.setOnClickListener{
            ...
            val img=baseUrl +data[position].photo
            // Glide.with(context).load(baseUrl +data[position].img).into(holder.img)
            val doc_name="Dr "+ data[position].name+" "+data[position].username
            ...
            val bundle = bundleOf("titre" to titre,"explication" to explication, "medicaments" to medicaments ,
                "date" to date, "doc" to doc_name,"lat" to lat,"lang" to lang,"tel" to tel,"adr" to adr,"img" to img,"idDoc" to idDoc)
